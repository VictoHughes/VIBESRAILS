#!/usr/bin/env bash
# VibesRails - Offline Installer (air-gapped environments)
# Usage: bash INSTALL.sh /path/to/project
#
# Place the .whl file in this directory before running.
set -euo pipefail
...
mkdir -p "$HOOKS_DIR"
if [ -f "$SCRIPT_DIR/../claude-code/hooks/ptuh.py" ]; then
    cp "$SCRIPT_DIR/../claude-code/hooks/ptuh.py" "$HOOKS_DIR/ptuh.py"
elif [ -f "$SCRIPT_DIR/ptuh.py" ]; then
    cp "$SCRIPT_DIR/ptuh.py" "$HOOKS_DIR/ptuh.py"
else
    echo "  (ptuh.py not found, skipping hook file copy)"
fi
