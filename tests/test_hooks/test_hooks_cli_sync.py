...
def _get_cli_flags() -> set[str]:
    ...
    # Use sys.executable to ensure we test the installed (editable) version
    import sys
    for cmd in [[sys.executable, "-m", "vibesrails", "--help"]]:
        try:
...
class TestTemplateSync:
    ...
    def test_template_structure_matches_project(self):
        """Project hooks.json matches the generated full tier structure.

        Since v2.4+, hooks are generated by hook_generator.py, not the static template.
        """
        if not PROJECT_HOOKS.exists():
            pytest.skip("No .claude/hooks.json in project")
        ...
        generated = build_hooks("full")

        # Same event types
        assert set(project["hooks"].keys()) == set(generated["hooks"].keys()), (
            "Hook event types differ between project and generated"
        )
